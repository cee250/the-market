...
export function PasswordResetPage() {
  ...
  const submit = async (e: FormEvent) => { e.preventDefault(); setBusy(true); setMessage(''); try { if (token) { if (password.length < 8 || password !== confirm) throw new Error('Passwords must match and be at least 8 characters.'); const result = await authApi.resetPassword(token, password); setMessage(result.message); toast('Password reset successfully.'); } else { const result = await authApi.requestPasswordReset(email.trim()); setMessage(result.message); toast('If the account exists, reset instructions are on the way.'); } } catch (err) { setMessage(err instanceof Error ? err.message : 'Unable to complete password reset'); } finally { setBusy(false); } };
